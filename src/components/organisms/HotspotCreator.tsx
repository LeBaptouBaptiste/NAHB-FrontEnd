...
export function HotspotCreator({
	imageUrl,
	initialHotspots = [],
	onSave,
	availablePages = [],
	...
	return (
		<div className="space-y-4">
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
				{/* Image Editor Area */}
				<div
					className="lg:col-span-2 relative bg-slate-900/50 rounded-lg border border-white/10 overflow-hidden"
					ref={containerRef}
				>
					{imageUrl ? (
						<div className="relative w-full h-full min-h-[400px]">
							...
							{containerDimensions.width > 0 &&
								hotspots.map((hotspot, index) => {
									const pixelRect = percentageToPixels(
										hotspot,
										containerDimensions
									);
									...
									return (
										<Rnd
											...
											} hover:border-emerald-400 transition-colors cursor-move`}
										>
				...
				<div className="space-y-4">
								...
								<div className="pt-4 border-t border-white/10 space-y-4">
									<div className="flex items-center gap-2">
										<input
											...
											onChange={(e) => {
												const updated = [...hotspots];
												const currentDiceRoll = updated[selectedId]
													.diceRoll || {
													enabled: false,
													difficulty: 10,
													type: "combat",
										...
										<div className="space-y-3 pl-6 border-l-2 border-emerald-500/30">
											<div className="space-y-2">
												...
												<Input
													type="number"
													value={
														hotspots[selectedId].diceRoll?.difficulty || 10
													}
													onChange={(e) => {
														const updated = [...hotspots];
														updated[selectedId] = {
											...
											<div className="space-y-2">
												...
												<select
													className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
													value={
														hotspots[selectedId].diceRoll?.type || "combat"
													}
													onChange={(e) => {
														const updated = [...hotspots];
														updated[selectedId] = {
											...
											<div className="space-y-2">
												...
												{availablePages.length > 0 ? (
													<select
														className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
														value={
															hotspots[selectedId].diceRoll?.failurePageId || ""
														}
														onChange={(e) => {
															const updated = [...hotspots];
															updated[selectedId] = {
					...
					<div className="text-xs text-muted-foreground p-2">
						<p>
							<strong>Tip:</strong> Drag to move, grab corners to resize.
						</p>
						<p>
							Zones are saved as percentages so they work on all screen sizes.
						</p>
					</div>
				</div>
			</div>
