...
const GamePlayer = () => {
	...
	const [diceConfig, setDiceConfig] = useState<{
		difficulty: number;
		type: "combat" | "flee" | "stealth" | "persuasion" | "custom";
	} | null>(null);
	...
	useEffect(() => {
		if (!session || !currentPage?.isEnding || pathStats || pathStatsLoading)
			return;
	...
	const handleChoice = async (choiceIndex: number, choiceText: string) => {
        ...
        const isLegacyDiceRoll = (choiceText.includes('1d20') || choiceText.includes('[COMBAT') || choiceText.includes('[FUITE')) && !isOutcomeChoice;
        ...
        if (isLegacyDiceRoll) {
			...
			const type =
				choiceText.includes("[FUITE") || choiceText.includes("Fuir")
					? "flee"
					: "combat";
	...
	const handleDiceResult = async (
		...
		if (pendingHotspot) {
			...
			const targetId = success
				? pendingHotspot.targetPageId
				: pendingHotspot.diceRoll?.failurePageId;

			if (targetId) {
				// Find choice leading to this target
				const choiceIndex = currentPage.choices.findIndex(
					(c) => c.targetPageId === targetId
				);
				if (choiceIndex !== -1) {
					await makeChoice(choiceIndex);
				} else {
					...
					console.warn(
						`No choice found for target ${targetId} after hotspot roll`
					);
				}
			}
    ...
    const makeChoice = async (choiceIndex: number, hotspotIndex?: number, diceRollSuccess?: boolean) => {
        if (!session || !currentPage) return;
		...
		try {
			...
			if (choiceIndex !== -1) {
				...
				if (
					choiceText.includes("potion") ||
					choiceText.includes("Prendre la potion")
				) {
					setInventory((prev) => [...prev, "potion"]);
				}
				if (
					choiceText.includes("médaillon") ||
					choiceText.includes("Prendre le médaillon")
				) {
				...
				if (choice.audio) {
					choice.audio.forEach((track) => {
						...
						if (track.type === "music") {
							...
							audio
								.play()
								.catch((e) => console.error("SFX playback failed:", e));
            ...
            const response = await gameService.makeChoice(session._id, choiceIndex, hotspotIndex, diceRollSuccess);
	...
	return (
		<div className="max-w-7xl mx-auto">
					...
					{playerClass && (
						<div className="bg-gray-800 px-4 py-2 rounded-lg border border-gray-700">
							...
							<span className="text-green-400 font-bold">
								{playerClass.combatBonus >= 0 ? "+" : ""}
								{playerClass.combatBonus}
								{statBuffs.combat > 0 && (
									<span className="text-yellow-400">+{statBuffs.combat}</span>
								)}
							</span>
							<span className="text-gray-400 text-sm ml-3">Fuite: </span>
							<span
								className={`font-bold ${
									playerClass.fleeBonus + statBuffs.flee >= 0
										? "text-green-400"
										: "text-red-400"
								}`}
							>
								{playerClass.fleeBonus >= 0 ? "+" : ""}
								{playerClass.fleeBonus}
								{statBuffs.flee > 0 && (
									<span className="text-yellow-400">+{statBuffs.flee}</span>
								)}
							</span>
						</div>
					)}
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
								...
								<div className="absolute inset-0 z-10">
									<HotspotLayer
										...
										onHotspotClick={async (hotspot: Hotspot) => {
											...
											const hotspotIndex = currentPage.hotspots?.findIndex(
												(h) =>
													h.x === hotspot.x &&
													h.y === hotspot.y &&
													h.width === hotspot.width &&
													h.height === hotspot.height
											);
				...
				<div
					className={`${
						currentPage.image ? "lg:col-span-1" : "lg:col-span-3"
					} space-y-6`}
				>
					{/* Page Content */}
						...
						<div className="prose prose-invert prose-emerald max-w-none text-gray-300">
							<ReactMarkdown
								...
								components={{
									...
									h4: ({ ...props }) => (
										<h4
											className="text-lg font-semibold text-emerald-400 mb-2"
											{...props}
										/>
									),
									p: ({ ...props }) => (
									...
									em: ({ ...props }) => (
										<em
											className="text-emerald-200 italic"
											{...props}
										/>
									),
									a: ({ ...props }) => (
									...
									blockquote: ({ ...props }) => (
										<blockquote
											className="border-l-4 border-emerald-500 pl-4 italic text-emerald-200 my-4 py-2 bg-emerald-500/5 rounded-r"
											{...props}
										/>
									),
									code: ({ className, children, ...props }) => {
										...
										return isInline ? (
											<code
												className="bg-gray-700 text-emerald-300 px-1.5 py-0.5 rounded text-sm font-mono"
												{...props}
											>
												{children}
											</code>
											...
											<code
												className="block bg-gray-900 text-emerald-300 p-4 rounded my-3 overflow-x-auto font-mono text-sm"
												{...props}
											>
												{children}
											</code>
									...
									table: ({ ...props }) => (
										<div className="overflow-x-auto my-4">
											<table
												className="min-w-full border border-gray-600 rounded"
												{...props}
											/>
										</div>
									),
									...
									th: ({ ...props }) => (
										<th
											className="border border-gray-600 px-4 py-2 text-emerald-400 font-semibold"
											{...props}
										/>
									),
									td: ({ ...props }) => (
                        ...
                        <div className="bg-gray-800 border border-gray-700 rounded-lg p-6 sticky top-6">
                            ...
                            <div className="space-y-3 max-h-[600px] overflow-y-auto pr-2">
                                {currentPage.choices.map((choice, index) => {
									...
									const isClassSelectionChoice =
										choice.text.includes("Choisir");
									...
									if (choice.condition?.type === "has_item") {
										...
										const hasItem = inventory.some(
											(item) =>
												item.toLowerCase() === requiredItem.toLowerCase()
										);
										if (!hasItem) {
										...
										<button
											...
											onClick={() => {
												...
												if (isClassSelectionChoice) {
													if (choice.text.includes("GUERRIER"))
														handleClassSelection("GUERRIER");
													else if (choice.text.includes("MAGE"))
														handleClassSelection("MAGE");
													else if (choice.text.includes("ASSASSIN"))
														handleClassSelection("ASSASSIN");
												}
							...
							<div className="mt-8 text-sm text-gray-300 text-left">
								{pathStatsLoading && (
									<p className="text-gray-400">
										Calcul des statistiques de votre parcours...
									</p>
								)}
								{pathStats && (
