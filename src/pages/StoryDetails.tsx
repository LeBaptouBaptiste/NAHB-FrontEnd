...
export function StoryDetails() {
	...
	const handleStartGame = async () => {
		...
		try {
			...
			const existing = await gameService.getSessionByStory(story._id);

			if (existing && existing.status === "in_progress") {
				setResumeSessionId(existing._id);
				setContinueModalOpen(true);
				return;
			}

			// 2. No previous session → start normally
			...
			const errorMessage =
				err.response?.data?.message || "Failed to start game session";

			if (errorMessage.includes("no start page")) {
				alert(
					"This story is not ready to play yet. The author needs to set up the story pages first."
				);
	...
	const handleSubmitReport = async () => {
		...
		try {
			...
			await reportService.reportStory(
				id,
				reportType,
							...
							<CardContent>
								<div className="text-muted-foreground text-sm">
									{Object.keys(story.stats?.endings || {}).length > 0
										? `${
												Object.keys(story.stats?.endings || {}).length
										  } unique endings discovered by players.`
