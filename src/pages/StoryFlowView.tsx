...
function StoryFlowViewInner() {
	...
	const onNodeClick = useCallback(
		(_event: React.MouseEvent, node: TStoryNode) => {
	...
	const handleUpdateContent = (content: string) => {
		...
		setSelectedNode((prev) =>
			prev ? { ...prev, data: { ...prev.data, content } } : null
		);
	};
	...
	const handleUpdateEnding = (isEnding: boolean) => {
		...
		updateNodeData(selectedNode.id, {
			isEnding,
			endingType: isEnding ? "neutral" : undefined,
		});
		setSelectedNode((prev) =>
			prev
				? {
	...
	const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		try {
			...
			setSelectedNode((prev) =>
				prev ? { ...prev, data: { ...prev.data, image: url } } : null
			);
		} catch (error) {
	...
	const handleRemoveImage = () => {
		...
		setSelectedNode((prev) =>
			prev ? { ...prev, data: { ...prev.data, image: undefined } } : null
		);
	};
									...
									{selectedNode.data.isEnding && (
										<div className="space-y-2">
											...
											<Select
												...
												onValueChange={(value) =>
													handleUpdateEndingType(
														value as "success" | "failure" | "neutral"
													)
												}
											>
