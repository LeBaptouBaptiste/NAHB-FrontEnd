...
export function Admin() {
	...
	const [actionType, setActionType] = useState<
		"ban" | "delete" | "approve" | "suspend" | "resolve" | null
	>(null);
	...
	const loadAdminData = async () => {
		try {
			...
			const [statsData, storiesData, usersData, reportsData] =
				await Promise.all([
	...
	const loadStories = async () => {
		try {
			const data = await adminService.getAllStories({
				search: searchQuery,
				limit: 50,
			});
	...
	const handleAction = (
		type: "ban" | "delete" | "approve" | "suspend" | "resolve",
		itemId: string
	) => {
		setActionType(type);
	...
	const statsDisplay = stats
		? [
				{
					label: "Total Stories",
					value: stats.stories.total.toString(),
					icon: BookOpen,
					change: "",
					color: "text-primary",
				},
				...
				{
					label: "Total Plays",
					value: stats.stories.totalViews.toLocaleString(),
					icon: TrendingUp,
					change: "",
															...
															<DropdownMenu>
																...
																<DropdownMenuContent align="end">
																	...
																	{story.status === "published" && (
																		<DropdownMenuItem
																			onClick={() =>
																				handleAction("suspend", story._id)
																			}
																		>
															...
															<div className="flex justify-end gap-2">
																{report.status === "pending" && (
																	<Button
																		size="sm"
																		onClick={() =>
																			handleAction("resolve", report._id)
																		}
																	>
																		Resolve
																	</Button>
										...
										<div className="space-y-4">
											...
											<div>
												<div className="flex justify-between text-sm mb-2">
													...
													<span>
														{stats.stories.totalViews.toLocaleString()}
													</span>
												</div>
												<div className="w-full h-2 bg-muted rounded-full overflow-hidden">
													<div className="h-full w-full bg-gradient-to-r from-primary to-secondary" />
