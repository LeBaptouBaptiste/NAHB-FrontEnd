...
export const aiService = {
	generateStory: async (data: AIStoryRequest) => {
		...
		const response = await api.post<{ data: { storyId: string } }>(
			"/ai/generate-story",
			data,
			{
	...
	suggestChoices: async (pageContent: string, storyContext: string) => {
		const response = await api.post<any[]>("/ai/suggest-choices", {
			pageContent,
			storyContext,
		});
		return response.data;
...
export const ratingService = {
	getStoryRatings: async (storyId: string, page = 1, limit = 10) => {
		const response = await api.get<RatingsResponse>(
	...
	getMyRating: async (storyId: string) => {
		const response = await api.get<Rating | null>(
			`/ratings/story/${storyId}/me`
		);
		return response.data;
	},
	rateStory: async (storyId: string, score: number, comment?: string) => {
		const response = await api.post<Rating>(`/ratings/story/${storyId}`, {
			score,
			comment,
		});
		return response.data;
...
export const adminService = {
	...
	toggleStorySuspension: async (storyId: string, suspended: boolean) => {
		const response = await api.patch(`/admin/stories/${storyId}/suspend`, {
			suspended,
		});
		return response.data;
	},
