...
export const useStoryFlow = (storyId: string | undefined) => {
	...
	const updateNodeData = useCallback(
		(nodeId: string, updates: Partial<StoryNodeData>) => {
			setNodes((nds) =>
				nds.map((node) => {
					if (node.id === nodeId) {
	...
	const deletePage = useCallback(async (pageId: string) => {
		try {
			...
			setEdges((eds) =>
				eds.filter((e) => e.source !== pageId && e.target !== pageId)
			);
				...
				pgs.map((page) => {
					if (page._id === params.source && params.target) {
						return {
							...
							choices: [
								...page.choices,
								{ text: "Next", targetPageId: params.target },
							],
						};
					}
					return page;
	...
	const updateEdgeLabel = useCallback(
		(edgeId: string, newLabel: string, sourceId: string, targetId: string) => {
			// Update visual edge
			setEdges((eds) =>
				eds.map((e) => {
					if (e.id === edgeId) {
					...
					if (page._id === sourceId) {
						return {
							...
							choices: page.choices.map((choice) =>
								choice.targetPageId === targetId
									? { ...choice, text: newLabel }
									: choice
							),
						};
					}
